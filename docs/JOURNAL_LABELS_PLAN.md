...
更新：2026-09-11。核心模型、卡片标签、组合筛选及校验规则已实现；索引证据仍在分批补全，未核实者保持待核验。
...
完成：字段与校验、默认全部范围、SCI/EI/双收录/待核验筛选、领域筛选、分区组合、直接可见标签、官网入口与边界测试。新增首本无已核实分区的中文 EI 补充条目。原始 19 本期刊已迁移字段；当前 65 本均采用该模型，只有取得明确来源的索引填写肯定值；全面核验仍待逐批推进。

以下为原实现批次记录，剩余核验顺序见 [主路线图](ROADMAP.md)。
