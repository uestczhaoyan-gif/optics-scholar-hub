...
更新：2026-09-14。结构化清单位于 [data/candidates.json](../data/candidates.json)，由原扩充计划和已有正式目录整理；此次整理不等于重新核实了所有候选官网。

当前 267 项：113 项 admitted、151 项 pending、3 项 deferred。初始整理为 262 项；数量随审核变化，请以生成报告为准。未收录候选不进入网站正式期刊数量或论文截止列表，也不自动抓取未知官网。
...
- relatedExistingIds：指向匹配类型的正式目录 ID。每个正式条目必须且只能关联一次；新届次也必须加入相应系列候选，否则数据校验失败。
...
4. 运行 `pnpm validate:data` 与 `pnpm report:coverage`。报告生成于 `source-report/coverage.md` 和 `.json`，Pages CI 随 maintenance-queue 附件提供，每日来源巡检也包含在 source-report 附件中。
